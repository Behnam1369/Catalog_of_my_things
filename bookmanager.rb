...
module BookManager
  def add_book(next_book_index)
    ...
    add_book(next_book_index) unless date_checker(publish_date)
    ...
    add_book(next_book_index) unless cover_state_checker(cover_state_selected)
  ...
  def date_checker(publish_date)
    begin
      ...
      puts 'Invalid date, it should be follow the formar: yyyy/mm/dd'
      return false
    end
    true
  end
